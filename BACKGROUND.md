...
Each response type has different latency requirements and triggers. Running them as separate concerns over one session keeps the live speech path fast while allowing slower generation to happen in the background.
...
- **Topic divergence**: if the presenter ignores the generated script and speaks about something else, the system detects drift and regenerates from the latest spoken context
